...
const PaperRockScissors = ({ onChoice, className, serverResponse, status }) => {
  ...
  return (
    <div className={`lg:w-full mx-auto shadow-md shadow-white w-[510px] rounded-3xl bg-base-300 p-12 text-[20px] lg:text-sm ${className}`}>
      <div className="mx-auto grid gap-8 grid-cols-3 text-center justify-between my-12 text-3xl">
        ...
        <div className="w-full gap-6 justify-between">
          <div className="flex gap-4 justify-between">
            <div onClick={() => handleChoice("paper")} className="bg-neutral w-24 h-24 aspect-square flex justify-center items-center hover:bg-secondary cursor-pointer z-50 ease-out duration-200 rounded-full p-2" aria-label="Choose Paper">
              <img src={paper} alt="Paper" className="sm:w-32 z-[10] object-cover" />
            </div>
            <div onClick={() => handleChoice("rock")} className="bg-neutral w-24 h-24 aspect-square flex justify-center items-center hover:bg-secondary cursor-pointer z-50 ease-out duration-200 rounded-full p-2" aria-label="Choose Rock">
              <img src={rock} alt="Rock" className="sm:w-32 z-[10] object-cover" />
            </div>
          </div>
          <div onClick={() => handleChoice("scissors")} className="bg-neutral w-24 h-24 mx-auto aspect-square flex justify-center items-center hover:bg-secondary cursor-pointer z-50 ease-out duration-200 rounded-full p-2" aria-label="Choose Scissors">
