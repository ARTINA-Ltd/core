...
const SimpleInput = ({ onChange, title, placeholder, type, isValid, validationError, defaultValue = "", disabled = false, className, ltr = false, options, menuPlacement, onKeyPress, maxChars }) => {
  ...
  const input = () => {
    if (type === "date" && disabled == false && defaultVal) {
      return <DatePicker accentColor="primary" onChange={onChange} inputClass={`simple-input  w-full`} defaultValue={defaultVal} className={`z-[3102] border-x-2 w-full border-x-transparent ${i18n.dir() === "rtl" ? "border-r-primary" : "border-l-primary"}`} dir={i18n.dir()} />;
    } else if (type === "date" && disabled == false && !defaultValue) {
      return <DatePicker accentColor="primary" position="center" round="x4" onChange={onChange} inputClass={`simple-input w-full`} className={`relative border-x-2 w-full border-x-transparent ${i18n.dir() === "rtl" ? "border-r-primary" : "border-l-primary"}`} dir={i18n.dir()} />;
    } else if (type === "date" && disabled == false) {
      return <DatePicker accentColor="primary" onChange={onChange} inputClass={`simple-input w-full`} className={`z-[3102] w-full border-x-2 border-x-transparent ${i18n.dir() === "rtl" ? "border-r-primary" : "border-l-primary"}`} dir={i18n.dir()} />;
    } else if (type === "date" && disabled == true) {
      return <input disabled={disabled} onKeyUp={onChange} type="text" className={`simple-input w-full border-x-2 border-x-transparent ${i18n.dir() === "rtl" ? "border-r-primary" : "border-l-primary"}`} placeholder={!focus ? "" : placeholder} defaultValue={new Intl.DateTimeFormat("fa").format(defaultVal)} onFocus={() => setFocus(true)} onBlur={(e) => (e.target.value === "" ? setFocus(false) : setFocus(true))} dir={i18n.dir()} />;
      ...
      return (
        <input
          ...
          value={value}
          type="text"
          className={`simple-input w-full translate-x-0 border-x-2 border-x-transparent ${i18n.dir() === "rtl" ? "border-r-primary" : "border-l-primary"}`}
      ...
      return <input disabled={disabled} onKeyUp={onChange} onChange={handleChange} value={value} type="text" className={`simple-input w-full border-x-2 border-x-transparent ${i18n.dir() === "rtl" ? "border-r-primary" : "border-l-primary"}`} placeholder={!focus ? "" : placeholder} defaultValue={defaultValue} onFocus={() => setFocus(true)} onBlur={(e) => (e.target.value === "" ? setFocus(false) : setFocus(true))} dir={i18n.dir()} />;
    } else if (type === "password") {
      return <input disabled={disabled} onKeyUp={onChange} type="password" className={`simple-input w-full border-x-2 border-x-transparent ${i18n.dir() === "rtl" ? "border-r-primary" : "border-l-primary"}`} placeholder={!focus ? "" : placeholder} defaultValue={defaultValue} onFocus={() => setFocus(true)} onBlur={(e) => (e.target.value === "" ? setFocus(false) : setFocus(true))} dir={i18n.dir()} />;
    } else if (type === "time") {
      return <TimePicker accentColor="primary" onChange={onChange} dir={i18n.dir()} />;
    } else if (type === "dropdown") {
      return <Select placeholder={placeholder} options={options} onChange={onChange} className={`simple-input2 w-full border-x-2 border-x-transparent ${i18n.dir() === "rtl" ? "border-r-primary" : "border-l-primary"}`} styles={customStyles} unstyled menuPlacement={menuPlacement} dir={i18n.dir()} />;
    } else {
  ...
  return (
    <div dir={i18n.dir()} className={`w-full items-start gap-1 relative $ bg-re{disabled ? "opacity-50" : ""} ${className} flex flex-col`}>
      ...
      <div
        dir={i18n.dir()}
        className={`absolute 
      ${i18n.dir() === "rtl" ? "left-2 top-1/2 -translate-y-1/2" : "text-center right-0 translate-y-1/2"}   text-sm bg-error text-error-content px-2 rounded-full font-b2 my-auto ${isValid ? "opacity-[0%]" : "opacity-[90%]"} transition-all`}
      >
        {validationError}
      </div>
    </div>
