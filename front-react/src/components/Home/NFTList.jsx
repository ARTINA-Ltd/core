...
const NFTList = ({ className }) => {
  ...
  useEffect(() => {
    ...
    axios
      .get("https://api.artina.org/api/transaction/nft_ratings/most_liked/")
      .then((d) => {
        setDataLiked(d.data);
      });
  }, []);
  ...
  return (
    ...
    >
      <div className="flex gap-6 bg-white rounded-t-xl px-3 pt-1">
        <div
          className={`cursor-pointer rounded-t-xl py-2 px-6 mt-2 ${
            selected ? "bg-gradient-to-b from-slate-200" : ""
          }`}
