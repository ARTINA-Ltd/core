...
export default function BuyTicketDialog({ onClick, exhibitionId, price, exhibitionName, hasLogin }) {
  ...
  const Header = (
    <div>
      <p className="font-b7">
        خرید بلیت نمایشگاه <span className="text-[#4e45d0]">"{exhibitionName}"</span>
      </p>
    </div>
  );
  ...
  return (
    <div className="card flex justify-content-center" onClick={onClick}>
      ...
      <Dialog header={Header} visible={visible} style={{ width: "50vw", direction: "rtl" }} onHide={() => setVisible(false)} footer={footerContent}>
        <div className="flex font-b4 gap-2 items-center w-full justify-between">
          <div className="flex gap-2 items-center">
            <div className="">قیمت بلیت:</div>
            <div className="bg-slate-100 hover:bg-slate-200 transition-all py-1 px-3 rounded-md duration-75 ease-out cursor-default">{price} تومان</div>
          </div>
        </div>
      </Dialog>
    </div>
